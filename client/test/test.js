...
it("Test 1: Add a Doctor", async () => {
    ...
    assert(await ehr.checkDoctor(doctor_id), true, "Doctor not added");
});
...
it("Test 2: Add a Patient", async () => {
    ...
    assert(await ehr.checkPatient(patient_id), true, "Patient not added");
});
...
it("Test 4: Delete a Doctor", async () => {
    ...
    await ehr.deleteDoctor(doctor_id, { from: admin_id });
    assert(!(await ehr.checkDoctor(doctor_id)), true, "Doctor not deleted");
...
it("Test 5: Delete a Patient", async () => {
    ...
    await ehr.deletePatient(patient_id, { from: admin_id });
    assert(!(await ehr.checkPatient(patient_id)), true, "Patient not deleted");
});
})